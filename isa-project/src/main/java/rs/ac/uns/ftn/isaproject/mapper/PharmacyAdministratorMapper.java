...
public class PharmacyAdministratorMapper {
	...
	public static Collection<PharmacyAdministratorDTO> toPharmacyAdministratorDTOs(Collection<PharmacyAdministrator> pharmacyAdministraotrs){
		...
		for(PharmacyAdministrator pa:pharmacyAdministraotrs) {
			pharmacyAdministratorDTOs.add(new PharmacyAdministratorDTO(pa.getId(), pa.getName(), pa.getSurname(), pa.getDateOfBirth(), pa.getEmail(), pa.getPassword(), pa.getAddress(), pa.getCity().getId(), pa.getCity().getName(), pa.getCity().getCountry().getName(), pa.getPharmacy().getId(), pa.getCity().getCountry().getId(), pa.getTelephone()));
		}
